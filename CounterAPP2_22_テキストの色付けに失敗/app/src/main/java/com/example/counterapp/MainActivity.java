...
public class MainActivity extends AppCompatActivity {
    ...
    //辞書を作成
    public String[] s_string = new String[81];//すべて 0 が格納
    int s_string_number = 0;

    public void demo_set(View v) throws InterruptedException {
...
//                //辞書を作成する
//                if(s[tate_][yoko_] != 0){
//                    s_string[s_string_number] = "s[" + String.valueOf(tate_) + "][" + String.valueOf(yoko_)+ "]";
//                    s_string_number++;
//                }
//            }
//        }
    ...
    public void set_number_table(View v){
        ...
        for(int tate_ = 1; tate_<=9 ;tate_++){
            for(int yoko_ = 1; yoko_<=9 ;yoko_++){
                textViews_s[tate_][yoko_] = findViewById(textview_button_[tate_][yoko_]);//ボタンテキストのIDをゲットする
        ...
        for(int tate_ = 1;tate_<=9; tate_++){
            for(int yoko_ = 1;yoko_<=9; yoko_++){
                ...
                //辞書と一致していたら文字を赤にする for文で探索
                for(int key_num = 0 ; key_num< s_string_number - 1;key_num++){
                    if(s_string[key_num] == "s[" + String.valueOf(tate_) + "][" + String.valueOf(yoko_)+ "]"){
                        textViews_button_s[tate_][yoko_].setTextColor(Color.RED);//テキストの色を変える
                    }
                }
